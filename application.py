...
from helpers import distances, Operation, orgdistance
...
def score():
    ...
    # Output comparison
    orig_matrix = orgdistance(s1, s2)
    original_matrix = [[0 for y in range(len(s2) + 2)] for y in range(len(s1) + 1)]
    for i in range(len(s1)):
        original_matrix[i + 1][0] = s1[i]
    ...
    return render_template("score.html", original_matrix=original_matrix, matrix=matrix, s1=s1, s2=s2, operations=transitions)
